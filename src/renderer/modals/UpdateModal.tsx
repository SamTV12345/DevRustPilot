...
import {setAppToUpdateModal, setModalOpen, setOpenAddModal} from "./ModalSlice";
...
export const UpdateModal:FC<ModalProps>  = ({headerText,children, onCancel, onAccept, cancelText,acceptText, onDelete})=>{
    const methods = useForm();
    const openModal  = useAppSelector(state=>state.modalReducer.openUpdateModal)
    const dispatch = useAppDispatch()


    return  openModal ? createPortal(
        <FormProvider {...methods}>
            <form id="defaultModal" tabIndex={-1} aria-hidden="true" onClick={()=>dispatch(setAppToUpdateModal(false))} onSubmit={
                ...
                <div className="grid place-items-center h-screen">
                    <div className="relative rounded-lg shadow bg-gray-700 justify-center w-full md:w-2/4" onClick={(e)=>e.stopPropagation()}>
                        <div className="flex justify-between items-start p-4 rounded-t border-b border-gray-600">
                            ...
                            <button type="button" className="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center hover:bg-gray-600 hover:text-white" data-modal-toggle="defaultModal" onClick={()=>dispatch(setAppToUpdateModal(false))}>
